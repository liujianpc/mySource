...
void Main_OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
    switch(id)
    {
        case IDC_OK:
			HWND hwndCheck = GetDlgItem(hwnd,IDC_CHECK1);
			int checkSt = Button_GetCheck(hwndCheck);
			if (checkSt==BST_CHECKED)
			{
				MessageBox(hwnd,TEXT("选中了选项1"),TEXT("hi"),MB_OK);
			}
			if (checkSt==BST_UNCHECKED)
			{
				MessageBox(hwnd,TEXT("未选中了选项1"),TEXT("hi"),MB_OK);
			}
